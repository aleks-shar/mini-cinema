...
@section('h1', 'Индивидуальные мета-теги')
...
@section('content')
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <form action="{{ route('individual-all') }}" method="get">
                        @csrf
                                ...
                                <table class="table table-striped projects">
                                    ...
                                    @foreach($tags as $item)
                                        <tr>
                                            ...
                                            <td>{{ \Illuminate\Support\Str::limit(AppView::getTitle($item->entity_id, $item->entity_type), 40)  }}
                                            </td>
                                            <td>{{AppView::getSeason($item->entity_id, $item->entity_type) }}</td>
                                            <td>{{AppView::getEpisode($item->entity_id, $item->entity_type) }}</td>
                                            <td>{{ AppView::getUserNameByEmail($item->email) }}</td>
                                            <td>{{ $item->updated_at }}</td>

                                            @if ((string)$item->entity_type === 'selection')
                                                <td>
                                                    <a href="{{ route('seo.individual.edit', [(string)$item->entity_type, (string)$item->uri, $item->entity_id]) }}"
                                            ...
                                            @endif
                                            <td>
                                                <form action="{{ route('seo.individual.destroy', [$item->id]) }}"
